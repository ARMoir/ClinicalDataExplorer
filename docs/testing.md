...
Browser smoke testing can use synthetic data with 23 patients and 12 observations to exercise 5/5/5/5/3 patient pages, name search and page reset, direct navigation, record sections, 10/2 observation pages, filtering and section selection. Use more than 50 records in a category to exercise its independent Load more control. Verify that a failed category can retry without clearing successful categories and that unopened categories remain marked as not loaded. Check the Units column, component values, collapsed document report text, and section order beneath Related persons. Census should initially hide empty locations; Clear filters should reveal them. Use a separate temporary content root/settings file for such fixtures; do not change deployed authentication or server settings.
...
`PatientResourceSearchTests` covers independent cursors, retry after failure, deduplication, empty continuation pages, cancellation, patient isolation, unsupported categories, unsafe/repeated links, supporting resources and document grouping. Patient-report compartment searches must also be verified against the target server.

`PatientEnrichmentTests` covers sequential background paging, foreground priority hooks, request-slot release, timeout with retained pages, unsupported operations, navigation cancellation, and merging missing identities without replacing foreground records. Browser verification should check the enrichment status, that new supporting sections appear without resetting expanded sections, and that category controls remain usable during enrichment.
