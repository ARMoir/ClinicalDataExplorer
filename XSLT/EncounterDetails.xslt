  ...
  <xsl:template match="/f:Encounter">
    <section class="xslt-report"><div class="xslt-report-heading"><div><span class="eyebrow">Encounter</span><h2><xsl:value-of select="f:type[1]/f:text/@value | f:type[1]/f:coding[1]/f:display/@value"/></h2></div><span class="status-chip"><xsl:value-of select="f:status/@value"/></span></div>
      <div class="xslt-detail-grid">
        <xsl:for-each select="f:participant/f:individual"><div class="xslt-wide"><span class="label">Provider</span><span class="value"><xsl:choose><xsl:when test="@p:provider"><xsl:value-of select="@p:provider"/></xsl:when><xsl:when test="f:display/@value"><xsl:value-of select="f:display/@value"/></xsl:when><xsl:otherwise><xsl:value-of select="f:reference/@value"/></xsl:otherwise></xsl:choose></span></div></xsl:for-each>
